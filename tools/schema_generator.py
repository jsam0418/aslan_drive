...
def generate_dataclass(table_name: str, table_def: Dict[str, Any]) -> str:
    ...
    lines.append(f'    """')
    lines.append(f'    {table_def.get("description", f"Data class for {table_name} table")}')
    lines.append(f'    """')
    ...
    for col_name, col_def in columns.items():
        ...
        lines.append(f'    {col_name}: {py_type}{default_val}  # {col_def.get("description", "")}')
...
def generate_sqlalchemy_model(table_name: str, table_def: Dict[str, Any]) -> str:
    ...
    lines = []
    lines.append('from sqlalchemy import Column, String, Date, DateTime, Numeric, BigInteger, Boolean')
    ...
    lines.append(f'    """')
    lines.append(f'    {table_def.get("description", f"SQLAlchemy model for {table_name} table")}')
    lines.append(f'    """')
    ...
    for col_name, col_def in columns.items():
        ...
        lines.append(f'    {col_name} = {col_def_str}  # {col_def.get("description", "")}')
...
def generate_sql_migration(schema: Dict[str, Any]) -> str:
    ...
    for table_name, table_def in tables.items():
        ...
        for index in table_def.get('indexes', []):
            ...
            lines.append(f'CREATE {index_type} IF NOT EXISTS {index["name"]} ON {table_name} ({cols_str});')
...
def main():
    ...
    parser.add_argument('--schema', default='schemas/market_data.json', 
    ...
    for table_name, table_def in schema['tables'].items():
        ...
        class_start = next(i for i, line in enumerate(lines) if line.startswith('class '))
