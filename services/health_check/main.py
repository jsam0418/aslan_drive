    ...
    try:
        ...
        with db_manager.get_session() as session:
            ...
            symbols_result = session.execute(
                text("SELECT DISTINCT symbol FROM daily_ohlcv WHERE date = :date ORDER BY symbol"),
                {"date": check_date}
            )
            symbols = [row[0] for row in symbols_result.fetchall()]
            
            # Get overall database statistics
            total_records_result = session.execute(text("SELECT COUNT(*) FROM daily_ohlcv"))
            total_records = total_records_result.scalar()
            
            total_symbols_result = session.execute(text("SELECT COUNT(*) FROM symbols WHERE active = true"))
            total_symbols = total_symbols_result.scalar()
            
            latest_date_result = session.execute(text("SELECT MAX(date) FROM daily_ohlcv"))
        ...
        logger.info(f"Health check passed: Found {record_count} records for {len(symbols)} symbols on {check_date}")
